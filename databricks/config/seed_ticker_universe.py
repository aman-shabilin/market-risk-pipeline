...
# MAGIC %md
# MAGIC # Seed the Ticker Universe
# MAGIC
# MAGIC Loads the S&P 500 constituent list into `ticker_universe`, which the ingestion
# MAGIC notebook reads when run with `ticker_source=table`. Run this once after
# MAGIC `setup_delta_tables`, and again whenever the index composition changes.
# MAGIC
# MAGIC The list is embedded below rather than fetched at runtime, so a run is
# MAGIC reproducible and reviewable in git — the alternative, scraping an index page on
# MAGIC every run, makes the pipeline's input depend on a third-party page's markup.
# MAGIC
# MAGIC **Two caveats worth knowing:**
# MAGIC - This is *today's* membership, so a long backfill over it is survivorship
# MAGIC   biased: companies that dropped out of the index are absent, which flatters
# MAGIC   aggregate returns. Fine for demonstrating a pipeline, wrong for backtesting.
# MAGIC - Symbols use Yahoo's share-class convention (`BRK-B`, not `BRK.B`).
# MAGIC
# MAGIC Source: S&P 500 constituents, retrieved 2026-08-30 from
# MAGIC `github.com/datasets/s-and-p-500-companies`.
...
# MAGIC ## Constituents
# MAGIC
# MAGIC `(yahoo_symbol, company_name, gics_sector)`
...
# MAGIC %md
# MAGIC ## Merge into `ticker_universe`
# MAGIC
# MAGIC Built as a DataFrame rather than a SQL `VALUES` literal: company names contain
# MAGIC commas, ampersands and apostrophes, and escaping 500 of them into a query string
# MAGIC is a bug waiting to happen.
# MAGIC
# MAGIC Symbols already in the table keep their `added_at`. Symbols in the table but no
# MAGIC longer in the list are marked `active = false` rather than deleted — their prices
# MAGIC stay valid history, they just stop being refreshed.
...
universe_df = (
    spark.createDataFrame(CONSTITUENTS, schema="ticker STRING, company_name STRING, sector STRING")
    .withColumn("active", F.lit(True))
    .withColumn("added_at", F.current_timestamp())
)

universe_df.createOrReplaceTempView("new_universe")

spark.sql(f"""
MERGE INTO {full_schema}.ticker_universe AS target
USING new_universe AS source
...
FROM {full_schema}.ticker_universe
GROUP BY sector
